...
def sort(list, filterName):
	...
	if filterName == "-AGE":
		newlist = sorted(list, key=lambda x: calculateAge(x.birthDate) if hasattr(x, 'birthDate') and x.birthDate else 0, reverse=True)
	if filterName == "+AGE":
		newlist = sorted(list, key=lambda x: calculateAge(x.birthDate) if hasattr(x, 'birthDate') and x.birthDate else 0)
