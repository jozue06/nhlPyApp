...
def run_test(test_name, query, expected_behavior=None):
    ...
    try:
        ...
        print(f"✅ SUCCESS - Found {player_count} players in {end_time - start_time:.2f}s")
...
def test_draft_filters():
    ...
    tests = [
        ("Ranked Players Only", "-RANKED", "Should show players with ranking data (currently shows all due to API limitations)"),
...
def test_country_filter():
    ...
    tests = [
        ...
        ("Multiple Countries", "-CODES \"USA,CAN\"", "Should show USA and Canada-born players"),
...
def test_rank_filters():
    ...
    tests = [
        ("Top 50 Ranked", "-MAX-RANK \"50\"", "Should show players ranked 50 or better (may not work with new API)"),
        ("Lower Ranked", "-MIN-RANK \"100\"", "Should show players ranked 100 or worse (may not work with new API)"),
    ]
...
def test_no_filters():
    ...
    return run_test(
        "No Filters - All Prospects", 
        "",
        "Should show all prospects from all teams"
    )
...
def main():
    ...
    for category_name, test_func in test_categories:
        ...
        if category_name in ["Help Menu", "No Filters"]:
            ...
            all_results.extend([(f"{category_name}: {name}", success, count) 
